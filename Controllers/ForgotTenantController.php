...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        header('Location: ../forgot_tenant.php');
        exit;
    }
    ...
    $stmt = $conn->prepare("SELECT * FROM tenants WHERE email = :email LIMIT 1");  // Assuming you have a tenants table
    ...
    $stmt = $conn->prepare("INSERT INTO tenant_resets (user_id, token, expiration) VALUES (:user_id, :token, :expiration)");
    ...
    try {
        ...
        $mail->Body    = "We received a request to reset your password. Click the link below to reset your password.<br>
                        <a href='localhost/poblacion/tenant-reset?token=$token'>Reset Password</a>";

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$token || !$newPassword) {
        ...
        header("Location: tenant-reset.php?token=" . urlencode($token));
        exit;
    }

    $db = new Database();
    $conn = $db->getConnection();
    $stmt = $conn->prepare("SELECT * FROM tenant_resets WHERE token = :token AND expiration > NOW()");
    ...
    $stmt = $conn->prepare("UPDATE tenants SET password = :password WHERE id = :user_id");
    $stmt->bindParam(':password', $newPasswordHash, PDO::PARAM_STR);
    $stmt->bindParam(':user_id', $userId, PDO::PARAM_INT);

    if ($stmt->execute()) {
        $stmt = $conn->prepare("DELETE FROM tenant_resets WHERE token = :token");
        $stmt->bindParam(':token', $token, PDO::PARAM_STR);
        $stmt->execute();
...
<main class="home flex flex-col items-center justify-center min-h-screen">
    <div class="relative flex flex-col m-6 space-y-8 bg-white shadow-2xl rounded-2xl md:flex-row md:space-y-0">
        <div class="flex flex-col justify-center p-8 md:p-14">
            ...
            ?>
            <form action="tenant-reset.php" method="POST">
                <div class="py-4">
